...
// start address in specified DMP bank #define DMP_REG             0x6F  //
// Register in DMP from which to read or to which to write #define DMP_REG_1
// 0x70 #define DMP_REG_2           0x71
...
#define BYTES2HWORD_BE(bytes) (((int16_t)(bytes)[0] << 8) | (bytes)[1])
#define BYTES2HWORD_LE(bytes) (((int16_t)(bytes)[1] << 8) | (bytes)[0])
...
void magCal(MPU9255_t* mpu, float* magBias, float* magScale) {
  uint16_t samples = 128;  // 1500 for 100Hz ODR and 128 for 8Hz (default)
  int16_t magMax[3] = {-32768, -32768, -32768},
          magMin[3] = {32767, 32767, 32767}, magTemp[3] = {0, 0, 0};
...
void MPU9255_ReadAccel(MPU9255_t* mpu) {
  ...
  mpu->Ax_raw = -BYTES2HWORD_BE(data);
  mpu->Ay_raw = BYTES2HWORD_BE(data + 2);
  mpu->Az_raw = BYTES2HWORD_BE(data + 4);
...
void MPU9255_ReadGyro(MPU9255_t* mpu) {
  ...
  /* DO NOT CHANGE THESE. MPU9255 REFERECE FRAME IS WEIRD */
  mpu->Gx_raw = BYTES2HWORD_BE(data);
  mpu->Gy_raw = -BYTES2HWORD_BE(data + 2);
  mpu->Gz_raw = -BYTES2HWORD_BE(data + 4);
...
void MPU9255_ReadMag(MPU9255_t* mpu) {
  ...
  if (I2C_ReadByte(hi2c, AK8963_ADDR, AK8963_ST1) & 0x01) {
    ...
    if (!(data[6] & 0x08)) {
      /* DO NOT CHANGE THESE. MPU9255 REFERECE FRAME IS WEIRD */
      mpu->My_raw = -BYTES2HWORD_LE(data);
      mpu->Mx_raw = BYTES2HWORD_LE(data + 2);
      mpu->Mz_raw = -BYTES2HWORD_LE(data + 4);
